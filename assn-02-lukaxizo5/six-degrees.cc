...
void generateShortestPath(const string& source, const string& target, const imdb& db) {
  ...
  while (!paths.empty() && paths.front().getLength() < 6) {
    path curr_path = paths.front();
    paths.pop();
    vector<film> movies;
    if (db.getCredits(curr_path.getLastPlayer(), movies)) {
      for (const film& movie : movies) {
        if (visited_movies.count(movie) == 0) {
          visited_movies.insert(movie);
          vector<string> cast;
          if (db.getCast(movie, cast)) {
            for (const string& actor : cast) {
              if (visited_actors.count(actor) == 0) {
                visited_actors.insert(actor);
                path curr_path_clone = curr_path;
                curr_path_clone.addConnection(movie, actor);
                if (actor == target) {
                  cout << endl << curr_path_clone << endl;
                  return;
                }
                else {
                  paths.push(curr_path_clone);
                }
              }
            }
          }
