...
(define (intersect-leftovers circle circles)
  (if (null? circles) '()
      (append (intersect circle (car circles))
              (intersect-leftovers circle (cdr circles))
      )
  )
)
...
(define (intersection-points circles)
  (if (null? circles) '()
      (append (intersect-leftovers (car circles) (cdr circles))
...
(define (distance-product point points)
  (apply * (map (lambda (curr)
                  (if (= (dist point curr) 0) 1
                      (dist point curr)
                  )
                )
              points
...
(define (rate-points points)
  (map (lambda (point)
          (list (distance-product point points) point)
       )
    points
  )
)
...
(define (clumped-points points)
  (map cadr (prefix-of-list (sort-points (rate-points points))
...
(define (average-point points)
  (let* ((len (length points))
         (x (/ (apply + (map car points)) len))
         (y (/ (apply + (map cadr points)) len))
         (point (make-pt x y))
         (rating (distance-product point points))
        )
  (list rating point)
  )
)
