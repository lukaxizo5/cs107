...
void getRandomSentence(const string& nonterminal, map<string, Definition>& grammar, vector<string>& randomSentence){
    if (grammar.find(nonterminal) == grammar.end()) {
      cout << "WRONG TERMINAL!\n";
      return;
    }

    Production production = grammar[nonterminal].getRandomProduction();
    for (auto it = production.begin(); it != production.end(); it++){
      const string& text = *it;
      if (text[0] != '<') {
        randomSentence.push_back(text);
      }
      else {
        getRandomSentence(text, grammar, randomSentence);
      }
    }
  }
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i <= 3; i++){
    ...
    getRandomSentence("<start>", grammar, randomSentence);
    for (const string& word : randomSentence) {
        cout << word << " ";
    }
    cout << "\n";
  }
